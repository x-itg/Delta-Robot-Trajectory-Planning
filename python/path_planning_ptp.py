...
class PathPlannerPTP:
    ...
    def point_to_point_345(self):
        ...
        theta_t = np.array(self.theta_i) + \
            np.array(self.theta_f - self.theta_i)*s_tau
    ...
    def trapezoidal_ptp(self, k=1/3):
        ...
        theta_t[T_a+1: -T_a-1] = a*T_a**2/2 + \
            thetadot_max*(t[T_a+1: -T_a-1] - T_a)
        theta_t[-T_a-1:] = a*T_a**2/2 + thetadot_max*(T - 2*T_a) + a/2*(
            T - T_a)**2 - a*T*(T - T_a) + a*T*t[-T_a-1:] - a*t[-T_a-1:]**2/2
    ...
    def plot_results(self, results):
        ...
        plt.subplot(511)
        plt.grid(True)
        plt.plot(tau, theta_t.transpose(), label=[
                 'theta_1', 'theta_2', 'theta_3'], linewidth=10)
        plt.title("angle-time plot", fontsize=20)
        plt.legend()
        ...
        plt.subplot(512)
        plt.grid(True)
        plt.plot(tau, theta_dot_t.transpose(), label=[
                 'theta_dot_1', 'theta_dot_2', 'theta_dot_3'], linewidth=10)
        plt.title("angular velocity-time plot", fontsize=20)
        plt.legend()
        plt.xlabel("normalized time", fontsize=15)
        plt.ylabel("angular velocity (deg/s)", fontsize=15)
        ...
        plt.subplot(513)
        plt.grid(True)
        plt.plot(tau, theta_ddot_t.transpose(),
                 label=['x', 'y', 'z'], linewidth=10)
        plt.title("angular acceleration-time plot", fontsize=20)
        plt.legend()
        plt.xlabel("normalized time", fontsize=15)
        plt.ylabel("angular acceleration (deg/s^2)", fontsize=15)
        ...
        plt.subplot(514)
        plt.grid(True)
        plt.plot(tau, theta_dddot_t.transpose(),
                 label=['x', 'y', 'z'], linewidth=10)
        ...
        plt.subplot(515)
        plt.grid(True)
        plt.plot(tau, ee_pos_t.transpose(), label=[
                 'x', 'y', 'z'], linewidth=10)
...
if __name__ == "__main__":
    ...
    # defining path planner
    path_planner = PathPlannerPTP(
        delta_robot, [0.00, -0.05, -0.42], [0.00, -0.05, -0.31], 20)
    # using 4567 planner
    results_4567_1 = path_planner.point_to_point_4567()

    # defining path planner
    path_planner = PathPlannerPTP(
        delta_robot, [0.00, -0.05, -0.31], [0.00, 0.00, -0.31], 20)
    # using 4567 planner
    results_4567_2 = path_planner.point_to_point_4567()

    # defining path planner
    path_planner = PathPlannerPTP(
        delta_robot, [0.00, 0.00, -0.31], [0.00, 0.00, -0.42], 20)
    # using 4567 planner
    ...
    for i in [results_4567_1, results_4567_2, results_4567_3]:
        # print("hi")
        (T_temp, theta_temp, thetadot_temp,
         thetaddot_temp, thetadddot_temp, eepos_temp) = i
